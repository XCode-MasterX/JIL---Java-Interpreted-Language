...
public class JILInt extends JILType{
    ...
    public void initFunctions() {
        ...
        functions.put("binary", (final int line, Object... args) -> {
            if(args.length > 0) 
                jil.error(line, "Too many arguments for the function that doesn't accept any." + line);
            
            final String x = binaryString();
            return new JILString(x, false);
        });

        functions.put("asString", (final int line, Object... args) -> {
            if(args.length > 0)
                jil.error(line, "Too many arguments. No arguments expected." + line);
