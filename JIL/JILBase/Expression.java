...
public class Expression {
    ...
    final static ArrayList<TokenType> subDelim;
    
    static { 
        subDelim = new ArrayList<>();
        subDelim.add(TokenType.RIGHT_PAREN);
        subDelim.add(TokenType.COMMA);
    }

    public Expression(final Block owner) {
        this.owner = owner;
        expression = new ArrayList<>();
        returnValue = null;
    }

    public int[] createExpression(final ArrayList<Token> statement, int index, int depth, final ArrayList<TokenType> delimiter) {
        line = statement.get(index).line;
        Token current = null;

        System.out.println(statement.get(index));
        for(; index < statement.size() && !delimiter.contains(statement.get(index).type); index++) {
            current = statement.get(index);

            if(current.type == TokenType.LEFT_PAREN) {
                Expression sub = new Expression(owner);

                int ret[] = sub.createExpression(statement, index + 1, depth + 1, subDelim);
                index = ret[0];
            ...
            else if(delimiter.contains(current.type)) break;
