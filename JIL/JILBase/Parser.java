...
public class Parser {
    ...
    public JILType parseExpression(final JILType variable, final ArrayList<Token> tokens, int index) {
        ...
        if(variable instanceof JILBoolean)              returnValue = parseExpAsBoolean(tokens, index);
        ...
        while(current.type != TokenType.NEWLINE) {            
            switch(current.type) {
                case TokenType.VARIABLE:
                    expression.add((JILType)interpreter.currentProgram.getVariable(current.lexeme).getCopy());
                    break;
    ...
    private JILDecimal parseExpAsDecimal(final ArrayList<Token> tokens, int index) {
        ...
        while(current.type != TokenType.NEWLINE) {            
            switch(current.type) {
                case TokenType.VARIABLE:
                    expression.add(interpreter.currentProgram.getVariable(current.lexeme).getCopy());
                    break;
                case TokenType.INT_CONSTANT:
